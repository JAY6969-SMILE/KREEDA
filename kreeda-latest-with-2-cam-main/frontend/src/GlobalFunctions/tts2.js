...
export default class TextToSpeech {
  constructor() {
    ...
    generateSpeech(
      "curl the weights upwards from Right hand too ",
      "Amy",
      "bc_rh"
    );
    generateSpeech(
      "curl the weights upwards from left hand too ",
      "Amy",
      "bc_lh"
    );
  ...
  player(index) {
    return playAudio(this.playlist[index]);
  }
  async generateSpeech(text, voiceid, name) {
    const params = {
      Text: text,
      OutputFormat: "mp3",
      VoiceId: voiceid,
    ...
    polly.synthesizeSpeech(params, (err, data) => {
      ...
      // Play the audio
      this.playlist[name] = data.AudioStream;
    });
  }
